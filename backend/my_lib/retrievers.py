    ...
    def setup_retrievers(self) -> None:
        ...
        try:
            self.retriever_small = BM25Retriever.from_documents(
                documents=self.small_chunks, k=self.top_k_BM25
            )
    ...
    def retrieve_small_chunks(self, shortened_question: str) -> list[Document]:
        ...
        try:
            ...
            scores = self.CE_model_keywords.predict(
                [
                    (shortened_question, result.page_content)
                    for result in small_chunks_retrieved
                ]
            )
        ...
        try:
            for chunk in small_chunks_retrieved:
                ...
                if score > 0:
                    ...
                    DRS_documents[name]["score"] = (
                        DRS_documents[name]["sum_score"] * log_plus_1
                    )
                elif not pos_score:
                    DRS_documents[name]["N"] += 1
                    score = 1 / abs(score)
                    DRS_documents[name]["score"] = max(
                        DRS_documents[name]["score"], score
                    )

            # sort documents according to DRS
            DRS_documents_sorted = sorted(
                DRS_documents.items(), key=lambda item: item[1]["score"], reverse=True
            )
            ...
            # selecting documents
            documents_selected = [
                doc[0] for doc in DRS_documents_sorted[: self.top_k_documents]
            ]
    ...
    def retrieve_large_chunks(self, question: str, files: list[str]) -> list[Document]:
        ...
        try:
            ...
            large_chunks_retrieved = self.vectorstore.similarity_search(
                query=question, k=self.top_k_semantic, filter=name_filter
            )
